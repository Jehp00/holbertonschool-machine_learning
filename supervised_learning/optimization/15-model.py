...
def create_batch_normal_layer(prev, n, activation):
    """creates a batch normalization layer"""
    W = tf.contrib.layers.variance_scaling_initializer(mode=('FAN_AVG'))
    model = tf.layer.Dense(units=n, name="layer", kernel_initializer=W)
    X = model(prev)
    beta = tf.Variable(tf.constant(0.0, shape=[n]), trainable=True)
    gamma = tf.Variable(tf.constant(1.0, shape=[n]), trainable=True)
    mean, variance = tf.nn.moments(X, [0])
    norm = tf.nn.batch_normalization(X, mean, variance, offste=beta,
                                     scale=gamma, variance_epsilon=1e-8)
    return activation(norm)

def create_Adam_op(loss, alpha, beta1, beta2, epsilon):
    """training operation with Adam optimization algorithm"""
    optimizer = tf.train.AdamOptimizer(alpha, beta1=beta1,
                                       beta2=beta2, epsilon=epsilon)
    return (optimizer.minimize(loss))

def create_layer(prev, n, activation):
    """tensor output"""
    W = tf.contrib.layersvariance_scaling_initializer(mode="FAN_AVG")
...
def forward_prop(x, layers=[], activations=[]):
    A = create_batch_normal_layer(x, layers[0], activations[0])
    for i in range(1, len(layers)):
        if i != len(layers) - 1:
            A = create_batch_normal_layer(A, layers[i], activations[i])
        else:
            A = create_layer(A, layers[i], activations[i])
    return A

def calculate_accuracy(y, y_pred):
    """calculates an accuracy of a prediction"""
    y_m = tf.argmax(y, axis=1)
    yp_m = tf.argmax(y_pred, axis=1)
    return tf.reduce_mean(tf.cast(tf.equal(y_m, yp_m), 'float32'))

def shuffle_data(X, Y):
    """shuffle the data point in two matrices the same way"""
